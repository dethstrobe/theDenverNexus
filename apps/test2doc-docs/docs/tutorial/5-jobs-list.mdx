...
import { enableVirtualAuthenticator, addPasskeyCredential, simulateSuccessfulPasskeyInput } from "@test2doc/playwright-passkey"
...
Now that we got a failing test, now we want to make it pass with a simple solution.
...
The test is still not passing. Let's add mock routes for these pages.
...
Let's update the `href` for the Logout link to our a new route.
