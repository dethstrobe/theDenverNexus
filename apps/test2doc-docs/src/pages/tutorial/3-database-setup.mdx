...
Let's to start by going through the [application design files](https://www.figma.com/design/nqBPIIfe0MO4W8zOfW4oae/Redwood-Reloaded-Tutorial?node-id=1-3&t=9gzbWUrBb2G3qdoa-1) and making a list of all the tables (models) and fields we need.
...
For our use case, we'll be giving the `Application` model a UUID and currently give other models' ids `Int`.
    ...
    async up(db) {
      return [
        await db.schema
          ...
          .addColumn('createdAt', 'integer', (col) => col.notNull().defaultTo(sql`strftime('%s', 'now')`))
    ...
    async up(db) {
      return [
        ...
        await db.schema
          ...
          .addColumn("id", "integer", (col) => col.primaryKey().autoIncrement())
          .addColumn('userId', 'text', (col) => col.notNull().unique().references('user.id')) 
          .addColumn('createdAt', 'integer', (col) => col.notNull().defaultTo(sql`strftime('%s', 'now')`))
        ...
        await db.schema
          .createIndex("Credential_credentialId_index")
          .on("Credential")
          .column("credentialId")
          .execute(),
        await db.schema
          .createIndex("Credential_userId_index")
          .on("Credential")
          .column("userId")
          .execute(),
      ]
...
- We'll be using singular names for our models. For example, `Application` not `Applications`.
- The model/table name should start with an uppercase letter, in *PascalCase*.
...
- The `createIndex` method needs a name, in case we want to later remove it or debug. We're following the naming convention `{TableName}_{columnName}_index` to help show what table it is for, what field is being indexed, and add `index` suffix just to make note this name is for indexing.
...
Let's look at this on the `Company` and `Contact` models:
...
```ts {4,19-21}
      ...
      await db.schema
        ...
        .addColumn("createdAt", "integer", (col) =>
          col.notNull().defaultTo("(strftime('%s', 'now'))"),
        )
        .addColumn("updatedAt", "integer")
        .execute()
      ...
      await db.schema
        ...
        .addColumn("companyId", "integer", (col) =>
          col.notNull().references("Company.id"),
        )
        .addColumn("createdAt", "integer", (col) =>
          col.notNull().defaultTo("(strftime('%s', 'now'))"),
        )
...
On the contact table's schema we add a reference to `"Company.id"` which adds a foreign key to the table that is associated with the company table's id field.
  ...
  "001_initial_schema_setup": {
    async up(db) {
      return [
        ...
        // 4. User (Credential depends on this)
        await db.schema
          .createTable("User")
          .addColumn("id", "integer", (col) => col.primaryKey().autoIncrement())
          .addColumn("username", "text", (col) => col.notNull().unique())
        ...
        // 5. Credential (Depends on User)
        await db.schema
          .createTable("Credential")
          .addColumn("id", "integer", (col) => col.primaryKey().autoIncrement())
          .addColumn("userId", "integer", (col) =>
            col.notNull().unique().references("User.id"),
          )
          .addColumn("createdAt", "integer", (col) =>
            col.notNull().defaultTo("(strftime('%s', 'now'))"),
          )
        ...
        // 6. Application (Depends on User, Company, ApplicationStatus)
        await db.schema
          .createTable("Application")
          .addColumn("id", "text", (col) => col.primaryKey())
          .addColumn("userId", "integer", (col) =>
          ...
          .addColumn("statusId", "integer", (col) =>
            col.notNull().defaultTo("1").references("ApplicationStatus.id"),
          )
          .addColumn("companyId", "integer", (col) =>
            col.notNull().references("Company.id"),
          )
...
export type Application = AppDatabase["Application"]
export type ApplicationStatus = AppDatabase["ApplicationStatus"]
export type Company = AppDatabase["Company"]
export type Contact = AppDatabase["Contact"]
...
This will seed the `ApplicationStatus` Table with all the statuses we'll be expecting. We can also add new migrations or more seed files later to populate the DB with more data, like for testing.
...
Then you can run `pnpm seed` which you'll see a in your root a new directory `.wrangler/state/v3/do/__change_me__-AppDurableObject/{randomNumbers}.sqlite`
...
Here area few helpful commands if you want to look around a bit more.
...
We should have a working database with relationships setup for when we need to use them next. Seeded the `ApplicationStatus` table, which will be used soon to show what state an Application is in.
